...
int	 Nflag, Pflag, rflag, sflag, Tflag, cflag;
...
#define	OPTIONS	"0123456789aBbC:cdD:efHhI:iL:nNPpqrS:sTtU:uwX:x:"
enum {
...
main(int argc, char **argv)
{
	...
	if (S_ISDIR(stb1.st_mode) && S_ISDIR(stb2.st_mode)) {
		...
		if (S_ISDIR(stb1.st_mode)) {
			argv[0] = ssplice(argv[0], argv[1]);
			if (stat(argv[0], &stb1) < 0)
				err(2, "%s", argv[0]);
		}
		if (S_ISDIR(stb2.st_mode)) {
			argv[1] = ssplice(argv[1], argv[0]);
