...
main(int argc, char **argv)
{
	...
	while ((ch = getopt_long(argc, argv, "aBbdEHI:ilo:stWw:",
		...
		case 't':
		case 'W':
			newdiffargv1  = malloc(sizeof(char) * strlen(diffargv[1]) + 2);
			diffargv[1]  = realloc(diffargv[1], sizeof(char) * strlen(diffargv[1]) + 2);
			/*
			 * In diff, the 'W' option is 'w' and the 'w' is 'W'.
			 */
			if (ch == 'W')
				sprintf(newdiffargv1, "%sw", diffargv[1]);
			else
				sprintf(newdiffargv1, "%s%c", diffargv[1], ch);
			free(diffargv[1]);
			diffargv[1] = newdiffargv1;
			break;
		case 'H':
